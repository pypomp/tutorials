...
def bench_dacca_mif(
    ...
    # add the parameter search box for searching multiple (NREPS_FITR) initial points 
    ...
    def _mif_loop(subkey):
        ...
        for i in range(reps):
            subkey, key_starts, key_mif, key_eval = jax.random.split(subkey, 4)
...
res_mif = bench_dacca_mif(J=NP_FITR, np_eval=NP_EVAL, M=NFITR, rw_sd = RW_SD, a=COOLING_RATE, reps=NREPS_FITR, reps_eval=NREPS_EVAL, seed=SEED)
