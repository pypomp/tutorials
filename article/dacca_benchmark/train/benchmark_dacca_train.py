...
def bench_dacca_train(
    ...
    dacca = pp.dacca(dt=None, nstep=20)
    ...
    def _train_loop(subkey):
        ...
        for i in range(reps):
            subkey, key_starts, key_train, key_eval = jax.random.split(subkey, 4)
            initial_params_list = pp.Pomp.sample_params(params_box, N_STARTS, key=key_starts)
            dacca.train(theta=initial_params_list, J=J, M=M, optimizer=optimizer, eta=eta_dict, scale=scale, ls=ls, n_monitors=n_monitors, key=key_train)
...
res_train = bench_dacca_train(J=NP_FITR, M=NTRAIN, optimizer=TRAIN_OPTIMIZER, eta=ETA, scale=TRAIN_SCALE, ls=TRAIN_LS, n_monitors=TRAIN_N_MONITORS, np_eval=NP_EVAL, reps_eval=NREPS_EVAL, reps=NREPS_FITR, seed=SEED)
