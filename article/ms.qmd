...
\note{introducing AD/JIT/GPU hardware + pypomp} As the demand grows for scalable and parallelizable inference algorithms, there is an increasing need for an accelerated framework for POMP modeling framework. `pypomp`[@abkemeier2024] addresses this by tightly integrating **automatic differentiation (AD)**, **just-in-time (JIT) compilation**, and **GPU/TPU accelaration** through JAX [@bradbury2018], a high-performance numerical computing library that supports hardware accelation and vectorization. AD techniques enable efficient and accurate derivative computation by systematically applying the chain rule to fundamental operations. Recent works have extended AD for gradient estimation using the particle filtering, yielding a class of methods termed automatic differetiation particle filters (ADPF). Building on these advancements, `pypomp` provides AD-enabled gradient estimation for inference for POMP models within a plug-and-play framework [@ionides2006], where users specify dynamic models solely through simulators of latent state trajectories, rather than evaluating the transition density of the latent Markov process. JAX's JIT compilation further accelerates repeated inference and evaluation by converting Python functions into machine code at runtime. Finally, JAX's GPU/TPU support enables efficient scaling of inference methods. Together, these features make `pypomp` more than a port of the `R` package `pomp`. Instead, it established an independent, modernized platform for fast and flexible POMP modeling in Python.
...
\note{Motivation for DMOP} Traditional standard particle filtering algorithms for POMP models provide unbiased Monte Carlo estimates, but they are non-differentiable with respect to model parameters, due to the discrete random sampling of particles. 
This prevents the AD and gradient-based optimization methods to be widely used in POMP dynamical systems. 
The differentiable measurement off-parameter particle filter (DMOP) [@tan2024], embedded in `pypomp`, provides a solution: it builds on a variant of the standard particle filter, called Measurement off-parameter-$\alpha$ (MOP-$\alpha$), which is a *differentiable* likelihood estimator. 
By using AD for obtaining the derivative, it doesn't necessarily require the final full likelihood estimate from MOP. 
There is a tunning dicount factor $\alpha \in [0,1]$ controlling the bias-variance trade-off of the gradient estimator. 
The derivative estimate from DMOP is essential for further stochastic gradient inference methods, such as iterated filtering with automatic differentiation (IFAD), where it employs stochastic gradient descent using DMOP. The pseudocode is shown in \ref{alg:dmop} 
...
The key building block of the DMOP method is the MOP-$\alpha$ algorithm.
It is a variant of the standard particle filter, making the final likelihood estimate differentiable with respect to the target parameter. 
To solve the issue that random resampling of particles in the classical particle filtering is normally discontinuous with respect to $\theta$, it evaluates the likelihood at the target evaluation parameter $\theta$ and resamples the particles with the indices generated by the standard particle filter at another baseline behavior parameter $\phi$.
This makes the Monte Carlo resampling process independent of the target evaluation parameter $\theta$, enabling the differentiability of the algorithm, and the further utlization of automatic differentiation and gradient descent.
The discount factor $\alpha \in [0,1]$, which controls the bias-variance tradeoff of the gradient estimator.
MOP-$\alpha$ returns the likelihood estimate. 
However, its significance lies not in its contribution to the likelihood estimate itself, but in collecting results that are differentiable with respect to the target parameters and then can be utilized by DMOP-$\alpha$. 
...
In `pypomp`, the DMOP is implemented by wrapping the JAX-compiled `mop()` function inside `jax.value_and_grad()`. 
This allows direct gradient computaiton with respect to $\theta$. 
The `mop()` function is internally run in a JAX-compiled function `_mop_internal()` but wrapped up into a class method. 
Similar to `pfilter`, it also uses static arguments and static array shapes supported by `jax.lax.fori_loop` to enhance efficiency. 
The weights and likelihood are also stored in a log scale to prevent underflow. 
Compared to MOP algoirthm, for DMOP and implementation, instead of setting two distinct parameters $\theta$ and $\phi$, the DMOP algorithm internally uses the `stop_gradient` function in `JAX` to prevent its input to be taken into the account for computing gradients. 
It also run the particle filter with $\theta = \phi$, and set the particles at $\phi$ to be copies of $\theta$ to avoid introducing new parameter values. 
