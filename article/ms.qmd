...
The key building block of the DMOP-$\alpha$ is the MOP-$\alpha$ algorithm, which obtains such stable and differentiable (log-)likelihood estimates discussed above.
It is a variant of the standard particle filter designed to make the discontinuous Monte Carlo resampling process independent of the target parameter $\theta$.
The algorithm evaluates the likelihood at the evaluation parameter $\theta$ but draws resampling indices using weights computed at another baseline behavior parameter $\phi$.
This renders the likelihood estimates differentiable w.r.t. $\theta$, further enabling AD.
In DMOP, MOP is run but by algebraically equivalently setting $\phi \equiv \texttt{stop\_gradient}(\theta)$ in JAX. 
MOP-$\alpha$ returns the likelihood estimate. 
Its significance lies not in its contribution to the likelihood estimate itself, but in creating a computational graph which is differentiable in target parameters and DMOP then differentiates to obtain the gradient estimate. 
...
In `pypomp`, DMOP-$\alpha$ is realized by wrapping the JAX-compiled `mop()` function inside JAX AD.
It is a `Pomp` class method calling an internal `_mop_internal()`, which uses static arguments and static array shapes supported by `jax.lax.fori_loop`, and keeps weights and likelihoods stored in log scale to prevent underflow. 
System sampling is involved for the resampling process. 
Users call `jax.grad` and `jax.value_and_grad` on `mop()` to obtain gradients. 

It is crucial to clarify how DMOP-$\alpha$ relates MOP-$\alpha$. 
The implementation of MOP algorithm alignes well with the likelihood evaluation in DMOP algorithm. 
Rather than defining another parameter to avoid gradient propagation, the DMOP algorithm internally calls JAX's `stop_gradient` to prevent its input to be differentiated. 
The particle filter runs $\theta = \phi$ by assuming the evaluation parameter and the bahavior parameter have the same values.
This avoids the need to explicitly introduce new parameters while preserving the differentiable property. 
Consequently, the likelihood evaluation of DMOP is identical to standard particle filter, and it does not require implement a fully separate MOP-$\alpha$ procedure.
...
Unlike `pfilter()`, which provides the choice of mutiple replicates and diagnostic summaries, `mop()` in `pypomp` focuses purely on producing differentiable likelihood estimates.
...
Besides a well-constructed `Pomp` object, the method requires only the number of particles `J`, the evaluation parameter `theta`, a PRNG key, and the cooling factor $\alpha$. 
The results are stored in the `results_history` attribute as a list of `jax.Array` objects, one for each parameter set, representing the estimated log-likelihood of the observations. For example, we run
