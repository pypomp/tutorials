...
The measurement off-parameter-$\alpha$ paricle filter (MOP-$\alpha$) algorithm [@tan2024].
It is a variant of the standard vanilla particle filter aiming to produce differentiable likelihood estimate with respect to the model parameters.
To solve the issue that random resampling of particles in the classical particle filtering is normally discontinuous with respect to $\theta$, it evaluates the likelihood at the target evaluation parameter $\theta$ and resamples the particles with the indices generated by the vanilla particle filter at another baseline behavior parameter $\phi$.
...
There is a discount factor $\alpha \in [0,1]$, which controls the bias-variance tradeoff of the gradient estimator.
The likelihood estimate is essential for other parameter inference algorithms such as iterated filtering with automatic differentiation (IFAD), where it employs stochastic gradient descient to MOP.
In `pypomp`, the MOP is implemented under `mop()`. The pseudocode is shown in \ref{alg:mop}. 
...
\begin{algorithm}[htbp!]
	\caption{DMOP-$\alpha$. \eic{THIS IS COPIED OVER FROM THE DMOP MS, AND IT NEEDS MODIFICATION FOR THE CURRENT LOCATION.}}
    \label{alg:dmop}
	     \textbf{Input:} Number of particles $J$, timesteps $N$, measurement model $f_{Y_n|X_n}(y_n^*|x_n, \theta)$, simulator $\process_n(\cdot|x_n; \theta)$, evaluation parameter $\theta$, baseline parameter $\phi$, seed $\omega$.
             ...
             \textbf{Forward pass:} AD constructs a computation graph for the likelihood estimate, $\hat{L}^A(\theta) = \prod_{n=1}^N L_n^{A,\theta,\alpha}$ or $\hat{L}^B(\theta) = \prod_{n=1}^N L_n^{B,\theta,\alpha}$. 
...
%            $L_n^{\phi} = \frac{1}{J}\sum_{m=1}^{J}g^{\phi}_{n,m}$.\newline
	    ...
	    \textbf{Return:} likelihood estimate $\hat{L}^A(\theta) = \prod_{n=1}^N L_n^{A,\theta,\alpha}$ or $\hat{L}^B(\theta) = \prod_{n=1}^N L_n^{B,\theta,\alpha}$, and gradient estimate $\hat{\mathcal{D}}^A(\theta)$ or $\hat{\mathcal{D}}^B(\theta)$.
\end{algorithm}
