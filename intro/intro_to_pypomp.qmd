...
We demonstrate by building the linear Gaussian model (available as `pypomp.models.LG()`) from scratch, using it to run iterated filtering 2 (IF2) and the particle filter, and visualizing the output.
...
In `pypomp`, model mechanics receive parameters as a dictionary (or `ParamDict`). 
...
The `Pomp` constructor also requires model parameters, provided as a `PompParameters` object. 
A `PompParameters` object can contain multiple parameter sets, in which case methods like `pfilter` will vectorize over each set.
Here, we use `Pomp.sample_params()` to generate a `PompParameters` object containing sets of parameters sampled from uniform distributions with bounds passed as a dictionary of length-2 tuples. 
...
theta = pp.Pomp.sample_params(param_bounds, n, subkey)
```
...
We run the `geometric_cooling()` method to set the cooling schedule for the random walk.

```{python Define the random walk standard deviations}
theta_names = LG_obj.canonical_param_names
rw_sd = pp.RWSigma(
    sigmas={k: 0.01 for k in theta_names}
...
```{python}
key, subkey = jax.random.split(key)
LG_obj.mif(J=200, M=200, rw_sd=rw_sd, key=subkey)
...
```{python}
LG_obj.traces().dropna(subset="logLik")
```

```{python}
(
    p9.ggplot(
        LG_obj.traces().dropna(subset="logLik").reset_index(),
