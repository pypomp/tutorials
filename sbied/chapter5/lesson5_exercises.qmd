...
format:
  beamer:
    ...
    output-file: exercises_slides.pdf
  pdf:
    ...
    toc: false
    geometry: margin=1in
    output-file: exercises_notes.pdf
...
measles_pomp = pp.models.UKMeasles.pomp(
    unit="London",
    theta=pp.PompParameters(theta_mle),
    model="001b",
...
# theta_idx, sim, time, and one column per state or observation
...
for sim_idx in range(n_sims):
    sim_data = Y_sims[(Y_sims[rep_col] == 0) & (Y_sims['sim'] == sim_idx)]
    sim_data = sim_data.sort_values('time')
    ax.plot(sim_data['time'].values, sim_data['cases'].values, 
...
ax = axes[1]
sim_data = Y_sims[(Y_sims[rep_col] == 0) & (Y_sims['sim'] == 0)]
sim_data = sim_data.sort_values('time')
ax.plot(sim_data['time'].values, sim_data['cases'].values, 
...
for sim_idx in range(n_sims):
    sim_data = Y_sims[(Y_sims[rep_col] == 0) & (Y_sims['sim'] == sim_idx)]
    sim_maxes.append(np.nanmax(sim_data['cases'].values))
...
for sim_idx in range(n_sims):
    sim_data = Y_sims[(Y_sims[rep_col] == 0) & (Y_sims['sim'] == sim_idx)]
    sim_means.append(np.nanmean(sim_data['cases'].values))
...
for sim_idx in range(n_sims):
    sim_data = Y_sims[(Y_sims[rep_col] == 0) & (Y_sims['sim'] == sim_idx)]
    sim_stds.append(np.nanstd(sim_data['cases'].values))
...
Here we compare likelihoods at different $\sigma_{SE}$ values:
...
for sigmaSE in sigmaSE_values:
    ...
    pomp_test = pp.models.UKMeasles.pomp(
        unit="London",
        theta=pp.PompParameters(theta_test),
        model="001b",
        first_year=1950,
        last_year=1963,
        dt=1/365.25,
    ...
    ll = pp.maths.logmeanexp(logliks)
...
Computing full profile likelihoods is computationally intensive.
Here we show the approach:
...
def compute_profile_point(cohort_value, theta_base, key):
    ...
    # Create POMP object
    pomp_obj = pp.models.UKMeasles.pomp(
        unit="London", theta=pp.PompParameters(theta_fixed), model="001b",
        first_year=1950, last_year=1963, dt=1/365.25, clean=True
    )
    
    # Run IF2
    pomp_obj.mif(J=2000, M=30, rw_sd=rw_sd, key=key, thresh=0)
    ...
    return {
        "cohort": cohort_value,
        "loglik": pp.maths.logmeanexp(logliks),
...
pomp_fixed = pp.models.UKMeasles.pomp(
    unit="London",
    theta=pp.PompParameters(theta_fixed_rho),
    model="001b",
    first_year=1950,
    last_year=1963,
    dt=1/365.25,
...
logliks = pf_result.logLiks.values.flatten()
ll_fixed = pp.maths.logmeanexp(logliks)
ll_fixed_se = pp.maths.logmeanexp_se(logliks)
...
When $\rho$ is fixed at a higher value (0.6 vs MLE $\approx 0.49$):
...
ll_mle = pp.maths.logmeanexp(pf_mle.logLiks.values.flatten())
