...
## Model specification {.allowframebreaks}
...
def rinit(theta_, key, covars, t0):
    ...
    I = jnp.round(m * I_0)
    R = jnp.round(m * R_0)
    return {"S": S, "E": E, "I": I, "R": R, "W": 0, "C": 0}
```
...
print(f"Date range: {dat['date'].min()} to {dat['date'].max()}")
...
display_df = pd.DataFrame({
    ...
    "amplitude": mle_df.loc["amplitude"].values,
    ...
    "sigmaSE": mle_df.loc["sigmaSE"].values,
}).sort_values("pop")

display_df = display_df.round({"R0": 1, "amplitude": 2,
...
params = ["R0", "amplitude", "LP", "IP", "iota", "rho",
          "psi", "sigmaSE"]
cors = {}
for p in params:
