---
title: "Chapter 5: Case Study -- Measles in Large and Small Towns \\vspace{5mm}"
author:
  - Aaron A. King
  - Edward L. Ionides
  - Kunyang He
subtitle: "[Source code at](https://github.com/pypomp/tutorials/tree/main/sbied/chapter5/source.qmd) [pypomp/tutorials/sbied/chapter5](https://github.com/pypomp/tutorials/tree/main/sbied/chapter5)"
date: today
...
format:
  beamer:
    ...
    header-includes: |
      ...
      % Shade code line by line instead of in one framed box, so that a long
      % code block can break across the frames of an allowframebreaks slide.
      \renewenvironment{Shaded}{\par\nointerlineskip}{\par}
      \renewcommand{\FancyVerbFormatLine}[1]{\makebox[0pt][l]{\hspace*{-\fboxsep}\textcolor{shadecolor}{\rule[-\dp\strutbox]{\dimexpr\linewidth+2\fboxsep\relax}{\baselineskip}}}#1}
...
mles = pp.models.UKMeasles.AK_mles()
...
## Map of the 20 cities in @He2010
...
The process model is implemented in `pypomp.models.measles.model_001b`.
...
transitions = pp.random.fast_multinomial(
    keys[2], populations, rt_final)
```
\normalsize
...
def rmeas(X_, theta_, key, covars, t):
    ...
    cases = jnp.where(cases > 0.0, jnp.round(cases), 0.0)
    return {"cases": cases}
```
\normalsize
...
```{python}
#| echo: true
london_data = pp.models.UKMeasles.subset(
    units=["London"])
dat = london_data["measles"]
print(f"Date range: {dat['date'].min().date()}",
    f"to {dat['date'].max().date()}")
...
* Periodic B-splines provide a flexible way to incorporate seasonality; the source code for `pp.models.dhaka` provides an example.
...
m1 = pp.models.UKMeasles.pomp(
    unit="London",
    theta=pp.PompParameters(theta_london),
    model="001b",
...
## Estimates from @He2010 {.allowframebreaks .t}
...
if os.path.exists(cache_file):
    ...
    m1.pfilter(J=[50,500,5000][RL], reps=[2,5,10][RL],
        key=key, thresh=0, CLL=True, ESS=True)
    pf = m1.results_history[-1]
    london_pf = {"logliks": pf.logLiks.values.flatten(),
        "CLL": pf.CLL(), "ESS": pf.ESS()}
    with open(cache_file, 'wb') as f:
        pickle.dump(london_pf,f)
london_logliks = london_pf["logliks"]
...
for i in range(3):
    sim = Y_sims[(Y_sims["theta_idx"] == 0) & (Y_sims["sim"] == i)]
    sim = sim.sort_values("time")
    axes[i+1].plot(sim["time"].values, sim["cases"].values,
...
sim_df = Y_sims100.pivot_table(
    index="time", columns="sim", values="cases"
)
...
df_cll = london_pf["CLL"] # theta_idx  rep  time  CLL
model_cll = (df_cll
    .groupby('time')['CLL']
    .mean().
    reset_index()
...
afig, ax = plt.subplots(figsize=(4.5, 2))
...
df_ess = london_pf["ESS"]
rep_col = 'rep' if 'rep' in df_ess.columns else 'replicate'
ess1 = df_ess.loc[df_ess[rep_col]==0,['ESS']]
fig, ax = plt.subplots(figsize=(4.5, 2))
...
).geometric_cooling(a=0.95)

m1.mif(J=2000, M=50, rw_sd=rw_sd,
...
#| echo: false
mle_df = pp.models.UKMeasles.AK_mles()
...
```{python}
#| echo: false
#| label: seasonal amplitude profile plot
seas_prof = pd.read_csv("data/amplitude.csv")
df = seas_prof
df1 = df[df.iloc[:,0].str.contains('London')]
x1 = df1['amplitude']
y1 = df1['loglik']
...
ax1.set_xlim(0.01, 1)
ax1.tick_params(labelbottom=False)
ax1.text(0.02, max_sm, "London", va='bottom', ha='center')

ax2.scatter(x2,y2,
    s=8, alpha=0.4, label='raw')
ax2.plot(mcap2.fit['parameter'],
    mcap2.fit['smoothed'],
    'r-', lw=2, label='MCAP smoothed')
max_sm = np.nanmax(mcap2.fit['smoothed'])
ax2.axhline(max_sm - mcap2.delta,
    color='blue', linestyle='--',
    label='MCAP cutoff')
ax2.set(xlabel='amplitude of term-time seasonality')
ax2.set_ylim(max_sm - 15, max_sm + 0.5)
ax2.grid(alpha=0.3)
ax2.spines['top'].set_visible(False)
ax2.spines['right'].set_visible(False)
...
ax2.text(0.02, max_sm, "Hastings", va='bottom', ha='center')
...
```{python}
#| echo: false
#| label: cohort effect profile plot
seas_prof = pd.read_csv("data/cohort.csv")
df = seas_prof
df1 = df[df.iloc[:,0].str.contains('London')]
x1 = df1['cohort']
y1 = df1['loglik']
...
```{python}
#| echo: false
#| label: R0 profile plot
R0_prof = pd.read_csv("data/R0profile.csv")
df = R0_prof
df1 = df[df.iloc[:,0].str.contains('London')]
x1 = df1['R0']
y1 = df1['loglik']
...
ax1.set_xlim(8, 100)
ax1.tick_params(labelbottom=False)
ax1.text(10, max_sm, "London", va='bottom', ha='center')
...
ax2.axhline(max_sm - mcap2.delta,
    color='blue', linestyle='--',
    label='MCAP cutoff')
ax2.set(xlabel='basic reproductive number, R0')
ax2.set_ylim(max_sm - 15, max_sm + 0.5)
ax2.grid(alpha=0.3)
ax2.spines['top'].set_visible(False)
ax2.spines['right'].set_visible(False)
ax2.set_xticks([]) 
ax2.set_xscale('linear')
ax2.text(10, max_sm, "Hastings", va='bottom', ha='center')
...
```{python}
#| echo: false
#| label: sigSE profile plot

def mean_euler(rate,dt=1/365):
    return(dt/(1-np.exp(-rate*dt)))

sigSE_prof = pd.read_csv("data/env_noise.csv")
df = sigSE_prof
df1 = df[df.iloc[:,0].str.contains('London')]
x1 = 1/np.sqrt(df1['eta']) # different scale used by He et al. (2010)
y1 = df1['loglik']
IP1 = mean_euler(df1['gamma'])*365
LP1 = mean_euler(df1['sigma'])*365
...
df2 = df[df.iloc[:,0].str.contains('Hastings')]
x2 = 1/np.sqrt(df2['eta']) # different scale used by He et al. (2010)
y2 = df2['loglik']
IP2 = mean_euler(df2['gamma'])*365
LP2 = mean_euler(df2['sigma'])*365
mcap2 = pp.mcap(
    parameter=np.log(x2.values),
    loglik=y2.values,
    level=0.95, span=0.6)
...
fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1,
    figsize=(5, 2.5), sharex=True)
fig.supylabel('Loglik (left). Duration (right)', fontweight='bold', 
    fontsize='small',x=0.02)
#fig.text(0.98,0.5,'Duration (days)', fontweight='bold', 
#    fontsize='medium', horizontalalignment='right',
#    rotation=90)

dur1 = ax1.twinx() # right axis for durations
...
ax1.plot(np.exp(mcap1.fit['parameter']),
    ...
    'r-', lw=2, label='MCAP smoothed')
dur1.plot(x1,IP1,
    'k-', lw=2, label='Infectious period')    
dur1.plot(x1,LP1,
    'k--', lw=2, label='Infectious period')    
max_sm = np.nanmax(mcap1.fit['smoothed'])
ax1.axhline(max_sm - mcap1.delta,
...
ax1.spines['top'].set_visible(False)
ax1.spines['right'].set_visible(False)
dur1.spines['top'].set_visible(False)
ax1.set_xscale('log')
ax1.set_xlim(0.004, 0.5)
ax1.tick_params(labelbottom=False)
ax1.text(0.01, max_sm, "London", va='bottom', ha='center')

dur2 = ax2.twinx() # right axis for durations
dur2.plot(x2,IP2,
    'k-', lw=2, label='Infectious period')    
dur2.plot(x2,LP2,
    'k--', lw=2, label='Infectious period')    
...
ax2.axhline(max_sm - mcap2.delta,
    color='blue', linestyle='--',
    label='MCAP cutoff')
ax2.set(xlabel=r'sigSE')
ax2.set_ylim(max_sm - 15, max_sm + 0.5)
ax2.grid(alpha=0.3)
ax2.spines['top'].set_visible(False)
dur2.spines['top'].set_visible(False)
ax2.spines['right'].set_visible(False)
ax2.set_xticks([]) 
ax2.set_xscale('log')
ax2.text(0.01, max_sm, "Hastings", va='bottom', ha='center')
