---
title: "Chapter 3: Likelihood-based Inference for POMP Models \\vspace{5mm}"
author:
  - Aaron A. King
  - Edward L. Ionides
  - Kunyang He
subtitle: "[Source code at](https://github.com/pypomp/tutorials/tree/main/sbied/chapter3/source.qmd) [pypomp/tutorials/sbied/chapter3](https://github.com/pypomp/tutorials/tree/main/sbied/chapter3)"
date: today
...
![Schematic diagram. The Monte Carlo technique called the **particle filter** is central for connecting the higher-level ideas of POMP models and likelihood-based inference to the lower-level tasks involved in carrying out data analysis.](flowchart.jpg){width=8cm}
...
**The likelihood function** [@Fisher1922] is:
$$
...
\tikzset{>={}}; % this is needed to implement the arrow redefinition
  ...
  \node (Np) at (0,7.5)  [draw,diamond,aspect=2.5,color=red] {\texttt{J particles}};
...
* As discussed in [Chapter 2](https://pypomp.github.io/tutorials/sbied/chapter2/slides.pdf), this allow for an overdispersed measurement distribution.
...
if os.path.exists(cache_file):
    ...
    with open(cache_file, 'wb') as f:
        pickle.dump((logliks, result_df), f)
...
```{python}
arr_str = np.array2string(logliks.reshape(2,-1),
    separator=' ',
    max_line_width=50,
    formatter={'float': lambda x: f'{x:.1f}'})
formatted_str = arr_str.replace('[', '').replace(']', '')
...
ll_est = pp.maths.logmeanexp(logliks)
ll_se = pp.maths.logmeanexp_se(logliks)
...
Alternatively, the `to_dataframe()` method used above returns the results as a data frame, with `logmeanexp` already applied:
...
def compute_likelihood_slice(
    ...
    for val in param_values:
        ...
        pomp_test = pp.Pomp(
            ...
            ys=ys, theta=pp.PompParameters(theta_test),
            statenames=statenames,
            t0=0.0, nstep=7,
            accumvars=("H",),
            covars=None)
        ...
        results.append({
            param_name: val,
            'loglik': pp.maths.logmeanexp(ll),
            'loglik_se': pp.maths.logmeanexp_se(ll)
        })
...
mu_IR_values = np.linspace(0.2, 2.0, [5,10,15][RL])
    ...
    for beta in beta_vals:
        for mu_IR in mu_IR_vals:
            ...
            mod = pp.Pomp(
                rinit=rinit, rproc=rproc,
                dmeas=dmeas, rmeas=rmeas,
                ys=ys, theta=pp.PompParameters(th),
                statenames=statenames,
                t0=0.0, nstep=7,
                accumvars=("H",),
                covars=None)
