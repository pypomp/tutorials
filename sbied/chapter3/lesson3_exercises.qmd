...
We use `jax.scipy.stats.nbinom.logpmf` for the measurement log-density and `pp.random.fast_nbinomial` from `pypomp` for the corresponding simulator.
...
print(f"  Time per 1000 particles: {slope * 1000 * 1000:.1f} microseconds")
```
...
lme_ll = pp.maths.logmeanexp(ll_array)
lme_se = pp.maths.logmeanexp_se(ll_array)
...
1. **Increase particles ($J$)**: More particles leads to lower variance and less bias
...
## Exercise 3.4: Likelihood Slice in eta Direction
...
def compute_likelihood_slice(param_name, param_values, base_theta,
    ...
    for val in param_values:
        ...
        pomp_test = pp.Pomp(
            ...
            ys=ys, theta=pp.PompParameters(theta_test), statenames=statenames,
            t0=0.0, nstep=7, accumvars=("H",), covars=None
        )
        ...
        results.append({
            param_name: val,
            'loglik': pp.maths.logmeanexp(logliks_arr),
            'loglik_se': pp.maths.logmeanexp_se(logliks_arr)
        })
...
3. **Non-identifiability**: The elongated ridge suggests that $\beta$ and $\eta$ are not fully identifiable from these data alone.
...
These features are **typical** of epidemiological models and motivate using profile likelihoods for inference, fixing some parameters at known values, and employing specialized optimization algorithms (like iterated filtering).
