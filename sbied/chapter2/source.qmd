...
The process simulator `rproc` must have arguments `X_`, `theta_`, `key`, 
`covars`, `t`, and `dt` (in that order), and return a dictionary of the 
updated state:
...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    I_new = I + dN_SI - dN_IR
    R_new = R + dN_IR
    H_new = H + dN_IR  # Accumulate incidence           
...
- Now, to include the observations in the model, we must write either a `dmeas` or an `rmeas` component, or both.
...
```python
def nbinom_logpmf(x, k, mu):
    x, k, mu = [jnp.asarray(v)
                 for v in (x, k, mu)]
    logp_zero = jnp.where(x==0, 0., -jnp.inf)
    safe_mu = jnp.where(mu==0., 1., mu)
    gl = jax.scipy.special.gammaln
    core = (gl(k+x) - gl(k) - gl(x+1)
      + k*jnp.log(k/(k+safe_mu))
      + x*jnp.log(safe_mu/(k+safe_mu)))
    return jnp.where(mu==0., logp_zero, core)

def rnbinom(key, k, mu):
    kg, kp = jax.random.split(key)
    lam = jax.random.gamma(kg,k) * (mu/k)
    return jax.random.poisson(kp, lam)
```
:::
...
def dmeas(Y_, X_, theta_, covars, t):
    ...
    mu = rho * X_["H"]
    return nbinom_logpmf(
        Y_["reports"], k, mu)

def rmeas(X_, theta_, key, covars, t):
    rho, k = theta_["rho"], theta_["k"]
    mu = rho * X_["H"]
    return jnp.array([rnbinom(key,k,mu)])
```
:::
...
sir_obj = pp.Pomp(
    ...
    t0=0.0, nstep=7, accumvars=("H",), ydim=1, covars=None
)
```

:::{.content-visible when-format="beamer"}
```python
theta = {"Beta": 7.5, "mu_IR": 0.5,
         "N": 38000., "eta": 0.03,
         "rho": 0.5, "k": 10.0}
...
# State variable names (H is an accumulator variable)
statenames = ["S", "I", "R", "H"]
...
sir_obj = pp.Pomp(
    ...
    accumvars=("H",),     # Accumulator variable (reset each observation)
    ydim=1,               # Dimension of observation vector
    covars=None
)
```
:::
...
n_sims = 20
key = jax.random.key(42)
X_sims, Y_sims = sir_obj.simulate(
    key=key, nsim=n_sims)
sim_df = Y_sims.pivot_table(
    index="time", columns="sim",
    values="obs_0")
```
:::
...
# Extract simulated reports
# Y_sims is a DataFrame with columns: replicate, sim, time, obs_0
sim_df = Y_sims.pivot_table(
    index="time", 
    columns="sim", 
    values="obs_0"
