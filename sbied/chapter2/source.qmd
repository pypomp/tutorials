---
title: "Chapter 2: Simulation of Stochastic Dynamic Models \\vspace{5mm}"
author:
  - Aaron A. King
  - Edward L. Ionides
  - Kunyang He
subtitle: "[Source code at](https://github.com/pypomp/tutorials/tree/main/sbied/chapter2/source.qmd)  [pypomp/tutorials/sbied/chapter2](https://github.com/pypomp/tutorials/tree/main/sbied/chapter2)"
date: today
...
2. We define a **compartmental model** for which we specify
...
5. We specify deterministic and stochastic compartmental models in
   `pypomp` using Euler‐method simulation.
...
- We set up notation applicable to general compartment models [@Breto2009]. \framebreak \begin{center}

<!---
## A flow diagram for the SIR model
--->
...
- To account for demography (births/deaths/migration) we include source and sink compartments, which may not be represented on the flow diagram. We write $\mu_{BS}$ for a rate of births into $S$, and denote mortality rates by $\mu_{SD}$, $\mu_{ID}$, $\mu_{RD}$.


##

- The rates may be either constant or varying. For a simple SIR model, the recovery rate $\mu_{IR}$ is a constant but the infection rate has the time-varying form
    $$\mu_{SI}(t)=\beta \, I(t),$$
    with $\beta$ being the _transmission rate_. For the simplest SIR model, ignoring demography, we set
...
\bigr],
$$
where $\operatorname{Binomial}(n,p)$ has mean $np$ and variance $np(1-p)$, with $p=\mu_{SI}\bigl(\tilde I(t)\bigr)\,\delta$.
...
We do not emphasize it here. 
Why? When would you prefer an implementation of Gillespie's algorithm over an Euler solution?
...
#url = "https://kingaa.github.io/sbied/stochsim/Measles_Consett_1948.csv"
...
## Implementing the SIR model in `pypomp` {.allowframebreaks}

- To implement the model in `pypomp`, we first need a **stochastic
...
In `pypomp`, model components are defined as Python functions with specific
argument signatures. The initial state simulator `rinit` must have arguments
`theta_`, `key`, `covars`, and `t0` (in that order), and return a dictionary
mapping state names to their initial values:
...
- Measles symptoms tend to be quite recognisable, and children with
  ...
  each week's `reports` as being related to the number of
...
- We need a variable to track these **daily** counts. We modify our
  model to add a variable $H$ to tally the true incidence.
...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    dN_stacked = pp.random.fast_binomial(
        key, n=jnp.array([S, I]), p=jnp.array([p_SI, p_IR]))
    dN_SI = dN_stacked[0]
    dN_IR = dN_stacked[1]
    S_new = S - dN_SI
    I_new = I + dN_SI - dN_IR
...
- Now, we'll model the data, conditional on the latent state, by a negative‑binomial variable:
  ...
  $$
  with mean $\rho\,H(t)$ and variance $\rho\,H(t)+\bigl(\rho\,H(t)\bigr)^2/k$.
...
The measurement density `dmeas` returns a log-density, and the simulator `rmeas` returns a dictionary keyed by observation name:
...
Basic epidemiological theory defines $\mathcal R_0$ to be the expected number of secondary infections caused by one primary infection introduced into a fully susceptible population.  

For an SIR infection we have $\mathcal R_0 \approx \tfrac{L}{A}$, where $L$ is host lifespan and $A$ is mean age of infection.
  
- Age‑stratified serology indicates $A \approx 4–5\;\text{yr}$
  [@Anderson1991]. Assuming $L \approx 60–70\;\text{yr}$ gives
...
sim_df = Y_sims.pivot_table(
    index="time", columns="sim", values="reports"
)
...
```{python}
#| echo: false
#| fig-cap: "SIR simulation (red) and 1948 Consett measles data (black). Clearly, this leaves something to be desired. In the exercises, you'll see if this model can do better."
fig, ax = plt.subplots(figsize=(3.5, 2.8))
ys["reports"].plot(ax=ax, lw=1.5, color="black", label="observed")
...
Fiddle with the parameters to see if you can't find a model for which the
...
measles data and an SEIR model. Perform simulations as above and
adjust parameters to get a sense of whether improvement is possible by
