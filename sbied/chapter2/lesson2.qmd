...
# Compartment models in \pkg{pypomp}
...
### Implementing the SIR model in \pkg{pypomp} {.allowframebreaks}

- To implement the model in \pkg{pypomp}, we first need a **stochastic
...
In \pkg{pypomp}, model components are defined as Python functions with specific
...
- Now, to include the observations in the model, we must write either a \code{dmeas} or an \code{rmeas} component, or both.
...
The measurement density `dmeas` must have arguments `Y_`, `X_`, `theta_`, 
`covars`, and `t` (in that order), and return a scalar log-likelihood:
...
The measurement simulator `rmeas` must have arguments `X_`, `theta_`, `key`,
`covars`, and `t` (in that order), and return a JAX array of observations:
...
sir_obj = pp.Pomp(
    ...
    accumvars=(3,),       # Index of H in statenames (reset each week)
