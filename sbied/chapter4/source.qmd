...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    dN_SI = pp.random.fast_binomial(
        key_SI, n=S, p=p_SI)
    dN_IR = pp.random.fast_binomial(
        key_IR, n=I, p=p_IR)
...
def rmeas(X_, theta_, key, covars, t):
    ...
    H = X_["H"]
    mu = rho * H
    reports = pp.random.fast_nbinomial(key, k, mu=mu)
...
#| echo: false
n_reps = traces_da.sizes['theta_idx']
...
for ax, var, lab in zip(
    ...
    if var in traces_da.coords['variable'].values:
        for r in range(n_reps):
            ax.plot(iterations,
                traces_da.isel(theta_idx=r).sel(
                    variable=var).values,
                alpha=0.4, lw=0.7)
...
if os.path.exists(cache_file):
    ...
    n_local = mif_result.traces_da.sizes['theta_idx']
