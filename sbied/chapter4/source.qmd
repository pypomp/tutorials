...
if os.path.exists(cache_file):
    ...
    rep_dim = 'theta_idx' if 'theta_idx' in mif_result.traces_da.sizes else 'replicate'
    n_local = mif_result.traces_da.sizes[rep_dim]
...
if os.path.exists(cache_file):
    ...
    mod.mif(J=2000, M=50,
        rw_sd=rw_sd_profile, a=0.5, key=key)
...
if os.path.exists(cache_file):
    ...
    mod.mif(J=2000, M=50,
        rw_sd=rw_sd_rho_profile, a=0.5, key=key)
...
if os.path.exists(cache_file):
    ...
    mod.mif(J=2000, M=100,
            rw_sd=rw_sd_exp_med, a=0.5, key=key)
    key = jax.random.key(610408800)
    mod.mif(J=2000, M=100,
            rw_sd=rw_sd_exp_small, a=0.5, key=key)
...
if os.path.exists(cache_file):
    ...
    for i, muIR_val in enumerate(muIR_grid):
        ...
        mod.mif(J=2000, M=100,
            rw_sd=rw_sd_muIR_med,
            a=0.5, key=key)
        key = jax.random.key(i * 5000 + 3000)
        mod.mif(J=2000, M=100,
            rw_sd=rw_sd_muIR_small,
            a=0.5, key=key)
