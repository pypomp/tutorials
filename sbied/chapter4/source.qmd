---
title: "Chapter 4: Iterated Filtering -- Principles and Practice  \\vspace{5mm}"
author:
  - Aaron A. King
  - Edward L. Ionides
  - Kunyang He
subtitle: "[Source code at](https://github.com/pypomp/tutorials/tree/main/sbied/chapter4/source.qmd) [pypomp/tutorials/sbied/chapter4](https://github.com/pypomp/tutorials/tree/main/sbied/chapter4)"
date: today
...
* For the measurement log-density we use `jax.scipy.stats.nbinom.logpmf`, with the standard reparameterization $p = k / (k + \mu)$.

* For the corresponding simulator we use `pp.random.fast_nbinomial` from `pypomp`.
...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    dN_stacked = pp.random.fast_binomial(key, n=jnp.array([S, I]), p=jnp.array([p_SI, p_IR]))
    dN_SI = dN_stacked[0]
    dN_IR = dN_stacked[1]
...
def dmeas(Y_, X_, theta_, covars, t):
    ...
    mu = rho * H
    p = k / (k + mu)
    return jax.scipy.stats.nbinom.logpmf(Y_["reports"], k, p)
...
def rmeas(X_, theta_, key, covars, t):
    ...
    H = X_["H"]
    mu = rho * H
    reports = pp.random.fast_nbinomial(key, n=k, mu=mu)
...
`ParTrans` ensures IF2 random walks respect parameter constraints.
Parameter perturbations for IF2 are added on the **estimation scale**.
...
When we later relax the assumption on $\mu_{IR}$, we will need a second transformation that also puts $\mu_{IR}$ on the log scale:

```{python}
#| label: parameter transforms exp
#| echo: true
def to_est_exp(theta):
    """Natural -> estimation, with log(mu_IR)."""
    return {
        "Beta": jnp.log(theta["Beta"]),
...
def from_est_exp(theta):
    """Estimation -> natural, with exp(mu_IR)."""
    return {
        "Beta": jnp.exp(theta["Beta"]),
...
par_trans_exp = pp.ParTrans(
    to_est=to_est_exp, from_est=from_est_exp)
```
...
measSIR = pp.Pomp(
    ...
    ys=ys, theta=pp.PompParameters(theta),
...
* The **initial value parameter** (IVP) $\eta$, designated by `init_names`, affects the dynamics only at time $t_0$, so it is perturbed only at time $t_0$.
...
if os.path.exists(cache_file):
    ...
    mod_local = pp.Pomp(
        rinit=rinit, rproc=rproc,
        dmeas=dmeas, rmeas=rmeas,
        ys=ys, theta=pp.PompParameters(theta_list),
        statenames=statenames,
        par_trans=par_trans,
        t0=0.0, nstep=7,
        accumvars=("H",),
    ...
    traces_da = mod_local.results_history.last().traces_da
    with open(cache_file, 'wb') as f:
        pickle.dump(traces_da, f)
```

## Iterated filtering diagnostics

```{python}
#| label: plot mif traces
#| echo: false
rep_dim = 'theta_idx' if 'theta_idx' in traces_da.sizes else 'replicate'
n_reps = traces_da.sizes[rep_dim]
...
if os.path.exists(cache_file):
    ...
    # Rebuild mod_local from mif endpoints
    rep_dim = 'theta_idx' if 'theta_idx' in traces_da.sizes else 'replicate'
    n_local = traces_da.sizes[rep_dim]
    traces = traces_da
    theta_endpoints = []
    for r in range(n_local):
        th = theta.copy()
        for var in ['Beta', 'rho', 'eta']:
            th[var] = float(
                traces.isel(**{rep_dim: r}).sel(
    ...
    mod_eval = pp.Pomp(
        rinit=rinit, rproc=rproc,
        dmeas=dmeas, rmeas=rmeas,
        ys=ys, theta=pp.PompParameters(theta_endpoints),
        statenames=statenames,
        par_trans=par_trans,
        t0=0.0, nstep=7,
        accumvars=("H",),
    ...
    mod_eval.pfilter(key=key, J=5000, reps=10)
    ...
    for i in range(n_local):
        ...
        rows.append({
            **theta_endpoints[i],
            'loglik': pp.maths.logmeanexp(lls),
            'loglik_se': pp.maths.logmeanexp_se(lls)
        })
...
print(f"Best log-likelihood:"
      f" {local_results['loglik'].max():.1f}")
print(f"(SE:"
      f" {local_results.loc[local_results['loglik'].idxmax(), 'loglik_se']:.1f})")
```
...
In `pypomp`, passing a list of parameter dicts (wrapped in `pp.PompParameters`) to `Pomp` causes `mif` and `pfilter` to run all replicates **in parallel on the GPU**.

The full source code is [online](https://github.com/pypomp/tutorials/blob/main/sbied/chapter4/source.qmd). In summary,
...
if os.path.exists(cache_file):
    ...
    eta_grid = np.linspace(0.01, 0.95, 40)
    n_prof = 15

    # Build ALL 40*15=600 starts at once
    theta_list = []
    eta_index = []
    ...
    # All 600 runs in parallel on GPU
    key = jax.random.key(1196696958)
    mod.mif(J=2000, M=50,
        rw_sd=rw_sd_profile.geometric_cooling(a=0.5), key=key)
    key = jax.random.key(1196696959)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_profile.geometric_cooling(a=0.3), key=key)

    key = jax.random.key(1196696960)
    mod.pfilter(key=key, J=5000, reps=10)
    pf = mod.results_history.last()
...
The `mcap` function in `pypomp` constructs a profile likelihood confidence interval that accounts for Monte Carlo error (noise).

* Here, the noise is small so MCAP is not essential. For larger and more complex models, substantial noise on the profile is unavoidable.
...
if os.path.exists(cache_file):
    ...
    rho_grid = np.linspace(0.02, 0.9, 20)
    n_prof = 15

    # Determine starting box from global search
    good = results_df[
        results_df['loglik'] > results_df['loglik'].max() - 20]
    Beta_lo = good['Beta'].min()
    Beta_hi = good['Beta'].max()
    eta_lo = max(good['eta'].min(), 0.01)
    eta_hi = min(good['eta'].max(), 0.99)

    # Build ALL 20*15=300 starts at once
    theta_list = []
    rho_index = []
    for rho_val in rho_grid:
        for j in range(n_prof):
            ...
            th["Beta"] = np.random.uniform(
                Beta_lo, max(Beta_hi, 80))
            th["eta"] = np.random.uniform(
                eta_lo, eta_hi)
            theta_list.append(th)
            rho_index.append(rho_val)
    ...
    # All 300 runs in parallel on GPU
    key = jax.random.key(2105684752)
    mod.mif(J=2000, M=50,
        rw_sd=rw_sd_rho_profile.geometric_cooling(a=0.5), key=key)
    key = jax.random.key(2105684753)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_rho_profile.geometric_cooling(a=0.3), key=key)
    key = jax.random.key(2105684754)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_rho_profile.geometric_cooling(a=0.3), key=key)

    key = jax.random.key(2105684755)
    mod.pfilter(key=key, J=5000, reps=10)
    pf = mod.results_history.last()
...
if os.path.exists(cache_file):
    ...
    mod = pp.Pomp(
        ...
        ys=ys, theta=pp.PompParameters(theta_list),
        statenames=statenames,
        par_trans=par_trans_exp,
        t0=0.0, nstep=7,
        accumvars=("H",),
        covars=None
    )
    # Simulated tempering: 3 stages
    key = jax.random.key(610408798)
    mod.mif(J=2000, M=100,
            rw_sd=rw_sd_exp.geometric_cooling(a=0.5), key=key)
    key = jax.random.key(610408799)
    mod.mif(J=2000, M=100,
            rw_sd=rw_sd_exp_med.geometric_cooling(a=0.5), key=key)
    key = jax.random.key(610408800)
    mod.mif(J=2000, M=100,
            rw_sd=rw_sd_exp_small.geometric_cooling(a=0.5), key=key)

    key = jax.random.key(610408801)
    mod.pfilter(key=key, J=5000, reps=10)
    pf = mod.results_history.last()
...
rw_sd_muIR_small = pp.RWSigma(
    sigmas={
        "Beta": 0.005, "mu_IR": 0.0,
        "N": 0.0, "rho": 0.0,
        "eta": 0.005, "k": 0.0
    },
    init_names=["eta"]
...
if os.path.exists(cache_file):
    ...
    muIR_grid = np.linspace(0.2, 2.0, 19)
    n_prof = 100
    profile_muIR = []

    # Batch by grid point (100 parallel per batch)
    # to avoid GPU OOM from 1900 simultaneous runs
    for i, muIR_val in enumerate(muIR_grid):
        ...
        # 4-stage tempering (100 parallel)
        key = jax.random.key(i * 5000)
        mod.mif(J=2000, M=100,
            rw_sd=rw_sd_muIR_profile.geometric_cooling(a=0.5),
            key=key)
        key = jax.random.key(i * 5000 + 1000)
        mod.mif(J=2000, M=100,
            rw_sd=rw_sd_muIR_profile.geometric_cooling(a=0.5),
            key=key)
        key = jax.random.key(i * 5000 + 2000)
        mod.mif(J=2000, M=100,
            rw_sd=rw_sd_muIR_med.geometric_cooling(a=0.5),
            key=key)
        key = jax.random.key(i * 5000 + 3000)
        mod.mif(J=2000, M=100,
            rw_sd=rw_sd_muIR_small.geometric_cooling(a=0.5),
            key=key)

        key = jax.random.key(i * 5000 + 4000)
        mod.pfilter(key=key, J=5000, reps=10)
        pf = mod.results_history.last()
...
**4.** `pypomp` API for IF2: `mif()` runs IF2, `RWSigma(...).geometric_cooling(a=...)` sets the perturbations and their cooling, and `traces_da` holds the parameter traces.

**5.** Passing a **list of parameter dicts** (as `pp.PompParameters`) to `Pomp` runs all replicates in parallel on the GPU.
