...
* The likelihood surface for nonlinear POMP models often contains nonlinear ridges and variations in curvature. 
    * This is inconsistent with the usual practice of independent priors derived from expert opinion on marginal distributions of parameters.
    * It is problematic for specification of "flat" or "uninformative" prior beliefs.
    * It adds difficulties to Bayesian computation.

##

* Expert opinion, i.e., "prior belief", can be treated as data for non-Bayesian analysis.
  
* Our primary task is to identify the information in the data under investigation, so it can be helpful to use methods that do not force us to make our conclusions dependent on quantification of prior beliefs.
...
for ax, var, lab in zip(
    ...
    if var in traces_da.coords['variable'].values:
        for r in range(n_reps):
            ax.plot(iterations,
                traces_da.isel(replicate=r).sel(
                    variable=var).values,
                alpha=0.4, lw=0.7)
...
* For each grid point, we launch multiple starts **in parallel**. We keep $\eta$ fixed by setting its random walk standard deviation to zero.
...
Plotting the results of the profile calculation reveals that, while some of the IF2 runs either become "stuck" on local minima or run out of opportunity to reach the heights of the likelihood surface, many of the runs converge on high likelihoods.
...
if os.path.exists(cache_file):
    ...
    # Use best results from global search as starts
    best_starts = results_df.nlargest(
        n_prof, 'loglik')
    ...
    for rho_val in rho_grid:
        for _, row in best_starts.iterrows():
            th = theta.copy()
...
## Global search with $\rho=0.6$ fixed
...
if os.path.exists(cache_file):
    ...
    # Build ALL 19*100=1900 starts at once
    theta_list = []
    muIR_index = []
    for muIR_val in muIR_grid:
        for j in range(n_prof):
            th = theta.copy()
    ...
    # All 1900 runs: 4-stage tempering in parallel
    key = jax.random.key(610408798)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_muIR_profile, a=0.5, key=key)
    key = jax.random.key(610408799)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_muIR_profile, a=0.5, key=key)
    key = jax.random.key(610408800)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_muIR_med, a=0.5, key=key)
    key = jax.random.key(610408801)
    mod.mif(J=2000, M=100,
        rw_sd=rw_sd_muIR_small, a=0.5, key=key)

    key = jax.random.key(610408802)
    mod.pfilter(key=key, J=5000, reps=10)
    pf = mod.results_history.last()
