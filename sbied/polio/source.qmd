...
def rmeas(X_, theta_, key, covars, t):
    ...
    rmeas returns a dict keyed by observation name.
    """
    rho = theta_['rho']
    tau = theta_['tau']
    IO = X_['IO']
...
polio.theta = pp.PompParameters(params_rpomp)
...
L1 = pp.maths.logmeanexp(logliks)
L1_se = pp.maths.logmeanexp_se(logliks)
...
for i in range(min(5, Nsim)):
    ...
    ax.plot(sim_data['time'], sim_data['cases'], alpha=0.3, linewidth=0.5)
...
polio.mif(J=Np, M=Nmif, key=key, rw_sd=rw_sd,
          theta=pp.PompParameters(theta_list))
...
ll_mif = pp.maths.logmeanexp(logliks[best_idx, :])
ll_mif_se = pp.maths.logmeanexp_se(logliks[best_idx, :])
...
polio.theta = pp.PompParameters(starts)
polio.mif(J=Np, M=Nmif, key=key, rw_sd=rw_sd)
