...
format:
  beamer:
    ...
    header-includes: |
      ...
      % print cell output at the same size as the code
      \makeatletter\renewcommand{\verbatim@font}{\scriptsize\ttfamily}\makeatother
    output-file: exercises_beamer.pdf
...
print("Covariate columns:", ", ".join(covars.columns))
...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    epsilon = jax.lax.cond(var_epsilon < 1e-6, lambda _: 1.0,
        lambda _: pp.random.fast_gamma(subkey, 1.0/var_epsilon) * var_epsilon,
        operand=None)
...
def rmeas(X_, theta_, key, covars, t):
    """Measurement simulator. Returns a dict keyed by observation name."""
...
#| echo: true
# Create Pomp object
data_filtered = data[(data['time'] > t0 + 0.01)
                     & (data['time'] < 1953 + 1/12 + 0.01)].copy()
...
polio = pp.Pomp(
    ys=ys,
    theta=pp.PompParameters(params_guess),
    rinit=rinit,
    rproc=rproc,
    dmeas=dmeas,
    rmeas=rmeas,
...
```{python}
#| echo: true
# Alternative: Also estimate initial baby susceptibles
# as fractions
...
polio.mif(J=Np, M=Nmif, key=key, rw_sd=rw_sd)
...
#| fig-width: 6
#| fig-height: 3
if hasattr(pf_result, 'CLL') and not pf_result.CLL().empty:
    import plotnine as pn
    
    df_cll = pf_result.CLL(average=True)
    
    cll_mean = df_cll['CLL'].mean()
    cll_std = df_cll['CLL'].std()
    
    p = (
        pn.ggplot(df_cll, pn.aes(x='time', y='CLL'))
        + pn.geom_line(color='blue', size=0.5)
        + pn.geom_hline(yintercept=cll_mean, color='red', linetype='dashed')
        + pn.geom_hline(yintercept=cll_mean - 2 * cll_std,
                        color='orange', linetype='dotted')
        + pn.labs(
            x='Year',
            y='Conditional Log-Likelihood',
            title='Identifying Potential Outliers (Mean and Mean - 2*SD)'
        )
        + pn.theme_minimal()
    ...
    threshold = cll_mean - 3 * cll_std
    outliers = df_cll.loc[df_cll['CLL'] < threshold, 'time'].values
...
#| fig-width: 6
#| fig-height: 3
if hasattr(pf_result, 'ESS') and not pf_result.ESS().empty:
    import plotnine as pn
    
    df_ess = pf_result.ESS(average=True)
    
    p = (
        pn.ggplot(df_ess, pn.aes(x='time', y='ESS'))
        + pn.geom_line(color='green', size=0.5)
        + pn.geom_hline(yintercept=Np/2, color='red', linetype='dashed')
        + pn.labs(
            x='Year',
            y='Effective Sample Size',
            title='Particle Filter ESS'
        )
        + pn.theme_minimal()
        + pn.theme(figure_size=(6, 3))
    )
    ...
    low_ess = np.sum(df_ess['ESS'] < Np/10)
    print(f"Times with ESS < Np/10: {low_ess} out of {len(df_ess)}")
else:
...
#| fig-width: 6
#| fig-height: 4
if hasattr(mif_result, 'traces') and not mif_result.traces().empty:
    import plotnine as pn
    
    df_traces = mif_result.traces()
    params_to_plot = ['rho', 'psi', 'sigma_env', 'logLik']
    
    df_long = df_traces.melt(
        id_vars=['theta_idx', 'iteration'],
        value_vars=[col for col in params_to_plot if col in df_traces.columns],
        var_name='parameter',
        value_name='value'
    )
    
    p = (
        pn.ggplot(df_long, pn.aes(x='iteration', y='value', group='theta_idx'))
        + pn.geom_line(alpha=0.5, size=0.5)
        + pn.facet_wrap('~parameter', scales='free_y', ncol=2)
        + pn.labs(
            x='Iteration',
            y='Value',
...
print("")
print("The cooling parameter 'a' controls how quickly"
...
for name, params in strategies.items():
    print(f"{name}:")
    print(f"  Np={params['Np']}, Nmif={params['Nmif']}, "
          f"Nstarts={params['Nstarts']}")
```
...
print(f"  Np={stage1['Np']}, Nmif={stage1['Nmif']}, "
      f"Nstarts={stage1['Nstarts']}")
print(f"  Goal: Find promising regions")
print("")

print("Stage 2 (refinement): 1 hour")
stage2 = {'Np': 2000, 'Nmif': 100, 'Nstarts': 10}
print(f"  Np={stage2['Np']}, Nmif={stage2['Nmif']}, "
      f"Nstarts={stage2['Nstarts']}")
...
- Check parameter traces for convergence using `traces()`
...
2. `rmeas` must return a dict keyed by observation name, e.g., `{'cases': value}`

3. Use `nstep=1` for discrete-time models

4. Multiple starting points: pass `theta=pp.PompParameters(list_of_dicts)` to `mif()`

5. Use `polio.prune(n=k)` to keep top k parameter sets

6. Access traces via `mif_result.traces()` (tidy DataFrame with columns `theta_idx`, `iteration`, `method`, `logLik` and parameters)
