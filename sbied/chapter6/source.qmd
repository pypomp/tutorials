---
title: "Chapter 6: Case Study -- Forecasting Ebola  \\vspace{5mm}"
author:
  - Aaron A. King
  - Edward L. Ionides
  - Kunyang He
subtitle: "[Source code at](https://github.com/pypomp/tutorials/tree/main/sbied/chapter6/source.qmd) [pypomp/tutorials/sbied/chapter6](https://github.com/pypomp/tutorials/tree/main/sbied/chapter6)"
date: today
...
np.random.seed(1331)
key_chain = jax.random.key(14641)
...
- @King2015 focused critical attention on the simple and therefore common practice of fitting deterministic transmission models to cumulative incidence data.
...
## Process model {.allowframebreaks .t}
...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    n_stacked = jnp.concatenate([
        jnp.atleast_1d(S),
        jnp.maximum(E, 0),
        jnp.atleast_1d(I)
    ])
    p_stacked = jnp.concatenate([
        jnp.atleast_1d(pS),
        jnp.repeat(pE, nstageE),
        jnp.atleast_1d(pI)
    ])
    draws = pp.random.fast_binomial(
        key, n=n_stacked, p=p_stacked)
    transS = draws[0]
    transE = draws[1 : 1 + nstageE]
    transI = draws[-1]
...
def dmeas(Y_, X_, theta_, covars, t):
    ...
    size = 1.0 / jnp.maximum(k, 1e-10)
    p = size / (size + mu)
    return jnp.where(
        k > 0,
        jax.scipy.stats.nbinom.logpmf(cases, size, p),
        jnp.where(
            mu > 0,
            cases * jnp.log(mu) - mu
...
def rmeas(X_, theta_, key, covars, t):
    ...
    N_EI = jnp.maximum(X_["N_EI"], 0.0)
    mu = rho * N_EI
    n = 1.0 / jnp.maximum(k, 1e-10)
    cases = pp.random.fast_nbinomial(key, n, mu=mu)
    return {"cases": cases}
```
...
def build_ebola_model(country="Guinea"):
    ...
    # saved profiles give parameter values
    ...
    pomp_obj = pp.Pomp(
        ...
        ys=ys, theta=pp.PompParameters(theta), t0=t0,
        dt=timestep, covars=None,
...
for j, country in enumerate(
    ...
    for i, par in enumerate(["R0", "k"]):
        df = cp[cp["profile"] == par]
        ax = axes[i, j]
        ax.scatter(df[par], df["loglik"],
            s=8, alpha=0.5)
        max_ll = df["loglik"].max()
...
for i in range(min(20, 200)):
    sd = Y_sims[(Y_sims[rep_col]==0)
                & (Y_sims['sim']==i)]
    ax.plot(sd['time'], sd['cases'],
...
- Let's quantify this using the `probe` method in `pypomp`.
  Each probe receives a dictionary of observation arrays and returns a number.


##

```{python}
#| label: probe growth_rate
#| echo: true
def growth_rate(obs):
    """Exponential growth rate via linear
    regression on log(cases)."""
    y = np.asarray(obs["cases"])
...
```{python}
#| label: probes quantiles
#| echo: true
def quantile_1(obs):
    """1st quartile of log cases."""
    y = np.maximum(np.asarray(obs["cases"]), 0.5)
...
## Running the probes {.allowframebreaks .t}
...
To incorporate **parameter uncertainty**, we use [**empirical Bayes**](https://en.wikipedia.org/wiki/Empirical_Bayes_method). 
...
sle_forecast = pp.Pomp(
    ...
    dmeas=dmeas, rmeas=rmeas,
    ys=sle.ys,
    theta=pp.PompParameters(param_samples),
    t0=sle.t0, dt=timestep,
    covars=None,
    statenames=sle.statenames,
    accumvars=("N_EI", "N_IR"))
```
* We set `theta` to a list of parameter vectors, wrapped in `pp.PompParameters`, which will trigger simulations at each parameter vector.
...
* See [the source code](https://github.com/pypomp/tutorials/blob/main/sbied/chapter6/source.qmd) for full details.
...
key_chain, key_fc = jax.random.split(key_chain)
cache_file = cache_dir + "/forecast-sims.pkl"
if os.path.exists(cache_file):
    with open(cache_file, 'rb') as f:
        Y_fc = pickle.load(f)
else:
    ...
    fcast = pp.Pomp(
        ...
        dmeas=dmeas, rmeas=rmeas,
        ys=forecast_ys, theta=pp.PompParameters(forecast_thetas),
        t0=last_week, dt=timestep,
        covars=None,
...
print(f"columns: {Y_fc.columns.tolist()}")
rep_col_fc = 'theta_idx'
```
...
**Exercise 6.1.** *The Sierra Leone outbreak.*
...
**Exercise 6.2.** *Decomposing the uncertainty.*
