...
def rproc(X_, theta_, key, covars, t, dt):
    ...
    draws = pp.random.fast_binomial(key, n=n_stacked, p=p_stacked)
    transS = draws[0]
...
def rmeas(X_, theta_, key, covars, t):
    ...
    return {"cases": pp.random.fast_nbinomial(key, n, mu=mu)}
```
...
rep_col = 'theta_idx' if 'theta_idx' in Y_sims.columns else 'replicate'
```
...
sle_forecast = pp.Pomp(
    rinit=rinit, rproc=rproc,
    dmeas=dmeas, rmeas=rmeas,
    ys=sle.ys, theta=pp.PompParameters(param_samples),
...
print(f"Forecast: {Y_fc.shape[0]} rows, "
      ...
      f"columns: {Y_fc.columns.tolist()}")
rep_col_fc = 'theta_idx' if 'theta_idx' in Y_fc.columns else 'replicate'
```
