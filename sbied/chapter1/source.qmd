---
title: "Chapter 1: Introduction to Simulation-based Inference for Epidemiological Dynamics  \\vspace{5mm}"
author:
  - Aaron A. King
  - Edward L. Ionides
  - Kunyang He
subtitle: "[Source code at](https://github.com/pypomp/tutorials/tree/main/sbied/chapter1/source.qmd) $\\newline$  [pypomp/tutorials/sbied/chapter1](https://github.com/pypomp/tutorials/tree/main/sbied/chapter1)"
date: today
...
- An [R-`pomp` version of this course](ttps://kingaa.github.io/sbied) is available.

- `pypomp` supports graphical processing unit (GPU) hardware and automatic differentiation.
...
A **POMP model** consists of data $y^*_1,\dots,y^*_N$ collected at times $t_1<\dots<t_N$, modeled as noisy, incomplete, and indirect observations of a Markov process $\{X(t), t\ge t_0\}$. This is also known as a **hidden Markov model** or a **state space model**.
...
## From math to algorithms for POMP models
...
**`mod.rinit`**, **`mod.rproc`**, **`mod.dmeas`** and **`mod.rmeas`** are internal `pypomp` wrappers around the user-supplied functions.
...
**`mop`**: An automatically differentiable "measurement off-parameter" particle filter, available as `pp.functional.mop`.
...
**`pp.models.dhaka`**: The historical Dhaka (Dacca) cholera data with the model of @King2008

**`pp.models.lg`**: A linear Gaussian example for validating Monte Carlo methods against exact Kalman filter calculations

**`pp.models.UKMeasles`**: Pre-vaccination measles in England and Wales with the model of @He2010

**`pp.models.spx`**:  A stochastic volatility model for the S&P 500 stock market index.
